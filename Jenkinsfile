    ...
    stages {
        stage('DEPLOY EKS CLUSTER') {
            steps {
                sh(script:"""\
                    cd /var/code/ansible
                    ansible-playbook eks-cluster.yaml -t deploy --extra-vars '{"clusterName":"${params.clusterName}","region":"${params.region}","eksRoleName":"${params.eksRoleName}","subnetIds":"${params.subnetIds}","securityGroupIds":"${params.securityGroupIds}","instanceType":"${params.instanceType}"}'
                    """,returnStdout: true)
            }
        }
        stage('DEPLOY application') {
            steps {
                sh(script:"""\
                cd /var/code/ansible
                ansible-playbook k8s-app.yaml -t deploy
                """,returnStdout: true)
            }
        }
    }
